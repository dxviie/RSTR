...
- `src/lib/prep/` — the **plot prep** tool: takes an exported SVG and dresses it for the machine (output page, paper outline, calibration markers, reversed-layer doubling). `reverse.ts` reverses SVG path direction so a layer can be plotted twice for denser ink. It also accepts multiple same-size SVGs (a file list, a folder, or the studio's frame-sequence zip — geared to the video export): frames are laid out as a grid across as many pages as needed, same-color layers are merged for easy pen selection in saxi, each frame gets its number in single-stroke Hershey digits, and the export is a zip of page SVGs. That zip also carries a `…_frame-template.svg` — page 1 plus one `frame-N` rectangle per grid cell — which [Framesheet Studio](https://tools.d17e.dev/tool/framesheet-studio) reads to turn photos of the plotted pages back into a looping animation.
