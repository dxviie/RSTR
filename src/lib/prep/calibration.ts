...
//  + optional compact mode: only the cal-pen-N and cal-half layers, packed
//    into the smallest rectangle that fits the lines and their labels
//
...
export interface CalibrationOptions {
	...
	rotated: boolean;
	/** only the cal-pen-N + cal-half layers, packed as small as possible */
	compact?: boolean;
...
export const buildCalibrationBlock = (options: CalibrationOptions): CalibrationBlock => {
	const { pageW, penCount, forExport, rotated, compact = false } = options;
	...
	const cpad = 1.2; // compact: tight inner padding
	const penLabelW = 3.6; // compact: gap + "P8" glyphs right of the H-lines
	const yRulerX = ipad + labelW + tickMaj;
	const hX1 = compact ? cpad : yRulerX + lineGap;
	...
	// pen line positions — symmetric around the ruler reference; compact
	// packs the H-lines from the top edge and the V-lines from the left
	const hY = (p: number) =>
		compact ? cpad + 1 + p * penPitch : hY0 + (p - (penCount - 1) / 2) * penPitch;
	const vX = (p: number) =>
		compact ? cpad + 1.3 + p * penPitch : xRulerX0 + (p - (penCount - 1) / 2) * penPitch;

	const vY1 = compact ? hY(penCount - 1) + 3 : xRulerY + lineGap;
	const vY2 = vY1 + lineLen;
	const vYhalf = vY1 + lineLen * 0.5;

	// ── bounding box (local) ───────────────────────────────────────────
	const bwLoc = compact
		? Math.max(hX2 + penLabelW, vX(penCount - 1) + 1.4) + cpad
		: Math.max(hX2 + 6, xRulerX0 + rulerR + 2);
	const bhLoc = compact ? vY2 + 4.2 : vY2 + 4 + ipad;
	...
	// bounding box outline (preview only) — yellow, matching the layer dot
	if (!forExport) {
		g += `<rect x="0" y="0" width="${bwLoc}" height="${bhLoc}"
      fill="rgba(255,176,0,0.04)" stroke="#FFB000" stroke-width="0.4" stroke-dasharray="2 1.5"/>`;
	}

	// header, circle and ruler layers — dropped entirely in compact mode
	if (!compact) {
