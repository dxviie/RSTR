<script lang="ts">
	...
	/** add the Framesheet Studio frame template as an extra file in the zip */
	let addFrameTemplate = $state(true);
	...
	/** the frame template's boxes sit on the per-cell paper outlines when those are drawn */
	const framesOnOutline = $derived(layPaper && paperOutlineSize === 'artwork');
...
`;
	...
	/**
	 * The layers every page of a multi-frame export shares: paper outline,
	 * calibration block and page boundary. Identical on every page (only the
	 * cell count varies on a partly filled last page), which is what makes them
	 * the registration ink Framesheet Studio can anchor every sheet on.
	 */
	const registrationLayers = (pageW: number, pageH: number, cellCount: number): string => {
		let out = '';
		if (layPaper) {
			let marks = '';
			if (paperOutlineSize === 'artwork') {
...
`;
	...
	/**
	 * The extra layer that turns page 1 of the export into a Framesheet Studio
	 * frame template: one `frame-N` rectangle per grid cell, row-major, so the
	 * numbering is the animation order. With paper outlines drawn per cell the
	 * rectangles trace them (templateFrameBoxes): printed marks Studio aligns
	 * every frame on. Studio hides them before matching the plotted ink, so the
	 * template must stay a full page — a bare grid of rectangles gives the
	 * registration search too little to lock onto.
	 */
	const frameTemplateLayer = (): string => {
		let rects = '';
		for (const [index, box] of templateFrameBoxes(grid, frameMargin, framesOnOutline).entries()) {
			rects +=
				`    <rect id="frame-${index}" inkscape:label="frame-${index}"` +
				` x="${numFmt(box.x, 4)}" y="${numFmt(box.y, 4)}"` +
				` width="${numFmt(box.w, 4)}" height="${numFmt(box.h, 4)}"/>\n`;
		}
		return `  <!-- ═══ Frame rectangles — read by Framesheet Studio, not meant to be plotted ═══ -->
  <g inkscape:label="frame-template" inkscape:groupmode="layer" id="layer-frame-template"
     fill="none" stroke="#7a5cff" stroke-width="0.2" stroke-dasharray="2 2">
${rects}  </g>

`;
	};

	const frameTemplateHeader = (): string =>
		`<!-- Generated by RSTR plot prep · frame template for Framesheet Studio.
     Page 1 of the export plus one rectangle per cell (frame-0 … frame-${grid.perPage - 1},
     row major = animation order), ${framesOnOutline ? "on each frame's paper outline" : "around each frame's artwork (no paper outline per frame)"}.
     Framesheet Studio hides those rectangles and anchors on the plotted ink
     around them, so this doubles as the reference for where every frame sits
     on the paper.
     ${grid.perPage} frames per page · ${pageTotal} page${pageTotal === 1 ? '' : 's'} · ${frames.length} frames in total.
     In Framesheet Studio: load this as the template, drop the photos of the
     plotted pages in page order, then set "frames used" to ${frames.length}. -->`;

	/** one output page of the multi-frame export, as a full SVG document */
	const buildPageSvg = (pageIndex: number, asFrameTemplate = false): string => {
		const [pageW, pageH] = page;
...
`;
		...
		out += registrationLayers(pageW, pageH, pageFrames.length);
		// the template is page 1 verbatim — same ink, so it anchors exactly — with
		// the frame rectangles appended for Framesheet Studio to read
		if (asFrameTemplate) out += frameTemplateLayer();
	...
	const exportMultiZip = () => {
		...
		if (addFrameTemplate) {
			entries.push({
				name: `${base}_frame-template.svg`,
				data: encoder.encode(buildPageSvg(0, true))
			});
		}
		downloadBlob(
		...
		<aside class="pane left">
			...
			<section class="panel-group">
				...
				{#if multi}
					...
					<label
						class="toggle-row"
						title="add an extra _frame-template.svg to the zip — page 1 with one frame-N rectangle per cell, the template Framesheet Studio needs to turn photos of the plotted pages back into an animation"
					>
						<input type="checkbox" bind:checked={addFrameTemplate} />
						<span class="layer-dot" style="background: #7a5cff"></span>
						frame template file
					</label>
				{/if}
				<label
