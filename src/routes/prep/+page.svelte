...
`;
	 ...
	 * numbering is the animation order. Framesheet Studio reads the rectangles
	 * for the frame geometry and hides them before matching the plotted ink, so
	 * the template must stay a full page — a bare grid of rectangles gives the
	...
	const frameTemplateLayer = (): string => {
		...
		for (let index = 0; index < grid.perPage; index++) {
			...
			rects +=
				...
				` x="${numFmt(cell.x + frameMargin, 4)}" y="${numFmt(cell.y + frameMargin, 4)}"` +
				` width="${numFmt(frameW, 4)}" height="${numFmt(frameH, 4)}"/>\n`;
		}
...
`;
	...
	const frameTemplateHeader = (): string =>
     ...
     row major = animation order). Framesheet Studio hides those rectangles and
     anchors on the plotted ink around them, so this doubles as the reference
     for where every frame sits on the paper.
